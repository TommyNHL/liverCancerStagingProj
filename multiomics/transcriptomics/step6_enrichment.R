...
myTopHits <- topTable(ebFit, adjust ="none", coef=1, number=12897, sort.by="logFC")
...
msigdbr_species()
mus_gsea <- msigdbr(species = "Homo sapiens") #gets all collections/signatures with mouse gene IDs
...
mus_gsea_c2 <- msigdbr(species = "Homo sapiens", # change depending on species your data came from
...
myGSEA.df <- myGSEA.df[c(18, 36),] %>% 
    mutate(phenotype = case_when(
        NES > 0 ~ "Diesel-Treated", 
        NES < 0 ~ "Healthy"))

# create 'bubble plot' to summarize y signatures across x phenotypes
ggplot(myGSEA.df[1:2,], aes(x=phenotype, y=ID)) +
